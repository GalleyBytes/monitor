...
import (
	...
	"github.com/galleybytes/monitor/manager/internal/webserver"
...
func getFlags() {
	...
	flag.StringVar(&mutatingWebhookConfigurationName, "mutating-webhook-configuration-name", "monitor", "Name of webhook resource")
	flag.StringVar(&secretName, "secret-name", "monitor-webhook-certs", "Name of the secret used to mount certs")
	flag.StringVar(&namespace, "namespace", "tf-system", "Namespace the service is deployed into")
	flag.StringVar(&serviceName, "service-name", "monitor-manager", "Name of the service to back up mutating webhook configuration")
	flag.StringVar(&monitorImage, "monitor-image", "ghcr.io/galleybytes/monitor:0.0.0-3-g8d7f3ce-dirty", "The monitor image to add to tf resource")
	flag.StringVar(&ignoreTag, "ignore-tag", "monitor.galleybytes.com/ignore", "Tag if present on tf resource webhook will not mutate")
	imagePullPolicyString := flag.String("image-pull-policy", "IfNotPresent", "Pull policy to add to monitor in tf resource")
	flag.StringVar(&apiServiceHost, "api", "http://terraform-operator-api.tf-system.svc", "TFO api host - proto://host:port")
	flag.StringVar(&monitorManagerServiceHost, "manager", "https://monitor-manager.tf-system.svc", "Monitor manager host - proto://host:port")
...
// boolp returns a bool pointer to the bool value passed in
...
func main() {
	...
	<-mgr.isReadyCh
	webserver.Run(tlsCertFilename, tlsKeyFilename, monitorImage, ignoreTag, clusterName, monitorManagerServiceHost, imagePullPolicy, apiServiceHost, apiUsername, apiPassword)
}
